using System;
using TicTacToe.Infrastructure.Core;
using UnityEditor;
using UnityEngine;

namespace TicTacToe.Infrastructure.Editor {
    /// <summary>
    ...
    public class AssetDatabaseAssetLoader : IAssetLoader {
        public T LoadAsset<T>(string assetName = "") {
            ...
            var guids = AssetDatabase.FindAssets($"{assetName} t:{typeof(T)}");
            switch (guids.Length) {
                case > 1:
                    Debug.LogWarning($"Multiple {typeof(T).Name} found! Returning the first match!");
                    break;
                case 0:
